...
const fallbackImage = iconPath + "/NoImage.jpg"
...
func postIconHandler(c echo.Context) error {
	...
	err = os.WriteFile(iconPath+"/"+userModel.Name+".jpg", req.Image, 0666)
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "Failed to create image at "+iconPath+"/"+userModel.Name+".jpg")
	}

	rs, err := tx.ExecContext(ctx, "INSERT INTO icons (user_id, icon_hash) VALUES (?, ?)", userID, fmt.Sprintf("%x", sha256.Sum256(req.Image)))
	if err != nil {
